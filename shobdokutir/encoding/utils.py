...
b2u_maps = {"|": "।", "Ô": "‘", "Õ": "’", "Ò": "“", "Ó": "”", "ª¨": "্র্য", "¤cÖ": "ম্প্র", "i¨": "র‌্য", "²": "ক্ষ্ম",
...
def _find_sub(src_string: str, main_char: Dict) -> str:
    """
    Finds the substrings that has unicode representations
    """
    if src_string in main_char:
        return src_string
    else:
        src_string = _find_sub(src_string[:-1], main_char)
        return src_string
...
def main():
    words = ["সন্তর্পৌনে", "দুর্গেশনন্দিনী", "অন্তরযন্ত্র", "অর্ন্নেরা", "বিশেষত্ব", "বৌদ্ধিক", "বঙ্কিমচন্দ্র", "চট্টোপাধ্যায়"]

    std = ["mš—‡c©Š‡b", "`y‡M©kbw›`bx", "Aš—ihš¿", "A‡bœ©iv", "we‡klZ¡", "‡eŠw×K", "ew¼gP›`«", "P‡Ævcva¨vq"]

    for uni, bijoy in zip(words, std):
        converted_to_uni = bijoy2unicode(bijoy)
        converted_to_bijoy = unicode2bijoy(uni)
        print("Given Bijoy word: ", bijoy)
        print("Converted to Unicode: ", converted_to_uni)
        print("Std unicode: ", uni)
        if converted_to_uni == uni:
            print("Matched")
        else:
            print("Not Matched")
        for c, w in zip(converted_to_uni, uni):
            print((hex(ord(c)), c), "\t", (hex(ord(c)), w))

        print()
        print("Given Unicode word:", uni)
        print("Converted to Bijoy: ", converted_to_bijoy)
        print("Std bijoy: ", bijoy)
        if converted_to_bijoy == bijoy:
            print("Matched")
        else:
            print("Not Matched")

        for c, w in zip(converted_to_bijoy, bijoy):
            print((hex(ord(c)), c), "\t", (hex(ord(c)), w))
        print()
